...
class ReceiveHistory:
    ...
    def __init__(
        ...
        *,
        ...
        self._closed = False
        # The audio is not this store's, but it is this store's index: a
        # clip is named on the row it belongs to, and this is the only place
        # that knows when that call was. So `prune` collects the names of
        # the clips whose rows it drops -- a clip must not outlive its row
        # -- and of those older than `clip_days`, and hands them to whoever
        # owns the files. See Transcriber.discard_clips, and the retention
        # notes at transcribe.DEFAULT_KEEP_CLIPS.
        self.clip_reaper = None
        self.clip_days = 0
        # Every clip on a row started before this has already been reaped,
        # so each pass reads only the rows that have aged past the cutoff
        # since the last one rather than every old row in the log.
        self._clips_reaped_before = 0.0
    ...
    def configure(self, *, enabled: bool, max_records: int, retention_days: int,
                  clip_days: int = 0) -> None:
        """Apply settings changes in place (the store outlives a save)."""
        self._enabled = enabled
        self.max_records = max_records
        self.retention_days = retention_days
        self.clip_days = max(0, int(clip_days))
        self.prune(force=True)
    ...
    def records(
        ...
        *,
        ...
        `offset` skips that many matching rows first, which is how the
        history page turns into pages: the second page is `limit` rows in.
        A plain OFFSET rather than a keyset -- it lets the page say "page 3
        of 742", and walking the `started` index a few thousand rows down is
        far cheaper than the LIKE scan the search box already pays for.
        ...
        rows = self._db.execute(
            f"SELECT * FROM calls {where} ORDER BY started DESC, id DESC LIMIT ? OFFSET ?",
            params,
        ).fetchall()
    ...
    def prune(self, *, force: bool = False) -> int:
        ...
        The clips of the rows dropped go with them, and so do the clips of
        rows older than `clip_days` -- those rows stay, and keep the clip's
        name, which is how the page can say the audio was removed rather
        than that there never was any.
        """
        now = self._clock()
        ...
        with self._db:
            if self.retention_days:
                ...
                where = "ended IS NOT NULL AND started < ?"
                params = (now - self.retention_days * DAY_S,)
                doomed += self._clips_where(where, params)
                cursor = self._db.execute(f"DELETE FROM calls WHERE {where}", params)
            ...
            if self.max_records and self.max_records > 0:
                ...
                if boundary is not None:
                    where, params = "started < ?", (boundary["started"],)
                    doomed += self._clips_where(where, params)
                    cursor = self._db.execute(f"DELETE FROM calls WHERE {where}", params)
                    removed += cursor.rowcount
            if self.clip_days:
                cutoff = now - self.clip_days * DAY_S
                if cutoff > self._clips_reaped_before:
                    doomed += self._clips_where(
                        "started >= ? AND started < ?", (self._clips_reaped_before, cutoff)
                    )
                    self._clips_reaped_before = cutoff
        if removed:
            logger.debug("pruned %d call(s) from the receive history", removed)
        if doomed and self.clip_reaper is not None:
            reaped = self.clip_reaper(doomed)
            if reaped:
                logger.debug("pruned %d clip(s) with the receive history", reaped)
        return removed

    def _clips_where(self, where: str, params) -> list[str]:
        """The clip names on the rows a WHERE clause selects."""
        rows = self._db.execute(
            f"SELECT DISTINCT clip FROM calls WHERE clip IS NOT NULL AND clip != '' AND {where}",
            params,
        ).fetchall()
        return [row["clip"] for row in rows]
