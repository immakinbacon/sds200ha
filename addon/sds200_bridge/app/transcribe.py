...
# only way to tell a good one from a plausible invention is to listen.
#
# Three settings bound the directory, and a clip goes when any of them says
# so. Each reads 0 as "no limit of this kind":
#
# * `keep_clips` -- a count, oldest first. It was a hardcoded 300, which on
#   a busy system is a couple of hours; reported as "the older recordings
#   don't play" from the real install, where the history runs to months.
# * `keep_clips_mb` -- a disk budget, oldest first. The thing the count was
#   standing in for: at 8kHz mono 16-bit a clip is ~16kB/s, so what 3000
#   clips cost depends on how long people talk.
# * `keep_clips_days` -- an age, enforced by the history, which knows when
#   each clip's call was (see ReceiveHistory.prune).
#
# And whatever these say, a clip never outlives its row: the history hands
# the names of the rows it prunes to `Transcriber.discard_clips`. So all
# three at 0 means "audio for every call still in the history", which is
# what "keep everything" has to mean if the disk is to stay finite.
DEFAULT_KEEP_CLIPS = 3000
MAX_KEEP_CLIPS = 1_000_000
DEFAULT_KEEP_CLIPS_MB = 1024
MAX_KEEP_CLIPS_MB = 1_000_000
DEFAULT_KEEP_CLIPS_DAYS = 0
MAX_KEEP_CLIPS_DAYS = 3650
...
class ClipStore:
    ...
    def __init__(self, directory: str = CLIP_DIR, keep: int = DEFAULT_KEEP_CLIPS,
                 max_bytes: int = DEFAULT_KEEP_CLIPS_MB * 1024 * 1024):
        self.directory = directory
        self.keep = keep
        self.max_bytes = max_bytes
        # Bytes on disk, kept current by every write and delete that goes
        # through here, so the size cap costs no stat per file per save.
        # None until first needed: one walk of the directory, then
        # arithmetic. See `size`.
        self._bytes: int | None = None

    def set_limits(self, *, keep: int, max_mb: int) -> int:
        """Apply new retention limits; returns how many clips that dropped.

        Pruned at once rather than on the next save, so lowering a number
        frees the disk when the setting is saved and not when the next call
        happens to come in. 0 for either means no limit of that kind.
        """
    ...
    def _unlink(self, name: str) -> bool:
        """Delete one clip, keeping the running total honest."""
        path = self.path(name)
        try:
    ...
    def kept(self, name: str | None) -> bool:
        """Whether a clip a row names is still on disk.

        A row keeps its clip name after `prune` has deleted the file: the
        name is how the history says "there was audio", and forgetting it
        would make a pruned clip indistinguishable from a call that never
        had one. This is how the page tells those apart.
        """
        return bool(name) and os.path.isfile(self.path(name))
    ...
    def append(self, call_id: int, name: str, pcm: bytes, rate: int) -> str | None:
        ...
        try:
            ...
            data = audio_tap.pcm_to_wav(have + pcm, rate)
            before = os.path.getsize(path)
            with open(path, "wb") as out:
                out.write(data)
            if self._bytes is not None:
                self._bytes += len(data) - before
    ...
    def discard(self, names) -> int:
        ...
        """
        # Already gone is the outcome asked for, so a failure is not counted
        # and not reported.
        return sum(1 for name in names if self._unlink(name))

    def prune(self) -> int:
        """Enforce the count and the disk budget, oldest first.

        Both bounds in one pass over one sorted listing: a clip goes while
        there are more than `keep` or they add up to more than `max_bytes`,
        and 0 for either switches that bound off. Age is not decided here --
        the history knows when each call was, and enforces `keep_clips_days`
        from its own prune.
        """
        names = self.names()
        over_count = self.keep and len(names) > self.keep
        over_size = self.max_bytes and self.size() > self.max_bytes
        ...
        for name in sorted(names, key=key):
            if not (
                (self.keep and remaining > self.keep)
                or (self.max_bytes and self.size() > self.max_bytes)
            ):
                break
