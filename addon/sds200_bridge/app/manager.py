...
class ScannerManager:
    ...
    async def _apply_locked(self, config: dict) -> dict:
        ...
        if self.transcriber is not None:
            ...
            dropped = self.transcriber.clips.set_limits(
                keep=transcribe_cfg.get("keep_clips", 0),
                max_mb=transcribe_cfg.get("keep_clips_mb", 0),
            )
            if dropped:
                logger.info("clip retention lowered: %d clip(s) deleted", dropped)
        ...
        if self.history is not None:
            self.history.configure(
                ...
                clip_days=(config.get("transcribe") or {}).get("keep_clips_days", 0),
            )
