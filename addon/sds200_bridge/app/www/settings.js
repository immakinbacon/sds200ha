...
(function (SDS) {
  ...
  class SettingsView {
    constructor({ onSaved } = {}) {
      ...
      this.keepClipsBounds = [0, 1000000];
      this.keepClipsMbBounds = [0, 1000000];
      this.keepClipsDaysBounds = [0, 3650];
      this.gsiBounds = [1, 30];
    ...
    async refresh() {
      ...
      this.keepClipsBounds = data.keep_clips_bounds || this.keepClipsBounds;
      this.keepClipsMbBounds = data.keep_clips_mb_bounds || this.keepClipsMbBounds;
      this.keepClipsDaysBounds = data.keep_clips_days_bounds || this.keepClipsDaysBounds;
    ...
    _buildTranscribe() {
      ...
      // Three bounds on the audio kept, and a clip goes when any of them
      // says so. Each one's 0 is "no limit of this kind" -- and the hard
      // floor under all three is the history itself: a clip is deleted with
      // its call, so "keep everything" means every call still in the log.
      const clipField = (label, bounds, hint) => {
        const wrap = el("label");
        ...
        wrap.appendChild(el("small", null, hint));
        grid.appendChild(wrap);
        return input;
      };
      this.sttKeepClips = clipField(
        "Keep audio for the last",
        this.keepClipsBounds,
        "Calls. The transcript stays as long as the history does; this is how many calls " +
          "back the ▶ button still has audio to play. 0 means no limit by count. Lowering " +
          "it deletes the oldest clips on save."
      );
      this.sttKeepClipsMb = clipField(
        "Audio disk budget (MB)",
        this.keepClipsMbBounds,
        "Once the clips add up to more than this, the oldest are deleted to make room. " +
          "Roughly 100 kB a call. 0 means no limit by size."
      );
      this.sttKeepClipsDays = clipField(
        "Delete audio after (days)",
        this.keepClipsDaysBounds,
        "Audio older than this is deleted while the call and its transcript stay in the " +
          "history. 0 means as long as the history keeps the call — audio is always " +
          "deleted with its call."
      );
    ...
    _applyGeneral(config) {
      ...
      this.sttKeepClips.value = transcribe.keep_clips ?? 3000;
      this.sttKeepClipsMb.value = transcribe.keep_clips_mb ?? 1024;
      this.sttKeepClipsDays.value = transcribe.keep_clips_days ?? 0;
    ...
    _collect() {
      ...
      return {
        ...
        transcribe: {
          ...
          // `|| undefined` would turn a deliberate 0 back into the default.
          keep_clips: this._count(this.sttKeepClips),
          keep_clips_mb: this._count(this.sttKeepClipsMb),
          keep_clips_days: this._count(this.sttKeepClipsDays),
          upsample: this.sttUpsample.checked,
        },
      };
    }

    /** A number field where 0 is a value, not an absence. Blank still means
     *  "leave it as it was". */
    _count(input) {
