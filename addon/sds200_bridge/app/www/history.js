...
(function (SDS) {
  ...
  class HistoryView {
    constructor({ onCreateAction } = {}) {
      ...
      this.total = 0;
      this.oldest = null;
      // Zero-based, PAGE_SIZE rows each. Every filter change goes back to
      // the first page: page 4 of one filter is not page 4 of another.
      this.page = 0;
      this.scanners = [];
      this._build();
    }
    ...
    _build() {
      ...
      // Under the list rather than beside the filters: "older" is where you
      // go after reading to the bottom of the page.
      this.pager = el("div", "pager");
      this.newerButton = el("button", "secondary", "‹ Newer");
      this.newerButton.type = "button";
      this.newerButton.addEventListener("click", () => this._turn(-1));
      this.olderButton = el("button", "secondary", "Older ›");
      this.olderButton.type = "button";
      this.olderButton.addEventListener("click", () => this._turn(1));
      this.pageLabel = el("span", "sub", "");
      this.pager.append(this.newerButton, this.pageLabel, this.olderButton);
      this.pager.hidden = true;
      this.root.appendChild(this.pager);
    }

    _pageCount() {
      return Math.max(1, Math.ceil(Math.max(this.total, this.records.length) / PAGE_SIZE));
    }

    async _turn(direction) {
      const page = Math.min(Math.max(0, this.page + direction), this._pageCount() - 1);
      if (page === this.page) return;
      this.page = page;
      await this.refresh();
      // A page turn reads from the top, whichever button was clicked.
      this.count.scrollIntoView({ block: "start" });
    }

    /** A filter changed: back to the first page, then fetch. */
    _refilter() {
      this.page = 0;
      this.refresh();
    }
    ...
    async refresh() {
      ...
      if (this.page) params.set("offset", String(this.page * PAGE_SIZE));
      ...
      try {
        ...
        // Past the end -- the log was cleared or pruned under us. Fall back
        // to the last page there is, rather than showing an empty one.
        if (!this.records.length && this.page && this.total) {
          this.page = this._pageCount() - 1;
          await this.refresh();
          return;
        }
        this.render();
    ...
    onReception(message) {
      ...
      if (message.event === "start") {
        ...
        if (index === -1) {
          ...
          this.total += 1;
          // On an older page the new call belongs at the top of the first
          // one, not here. The count still moves, so the page label stays
          // true; the rows shift by one on the next turn, which is what
          // paging a live log means.
          if (this.page) {
            this._renderPager();
            return;
          }
    ...
    render() {
      ...
      const last = this.page * PAGE_SIZE + this.records.length;
      const shown =
        total > this.records.length
          ? `${first.toLocaleString()}–${last.toLocaleString()} of ${total.toLocaleString()} calls`
    ...
    _renderPager() {
      ...
      this.pageLabel.textContent = `Page ${this.page + 1} of ${pages.toLocaleString()}`;
      this.newerButton.disabled = this.page === 0;
      this.olderButton.disabled = this.page >= pages - 1;
    }
     ...
     *  was right to refuse.
     *
     *  A clip the retention setting has since deleted is said so, in words,
     *  rather than shown as a button that fails when pressed: the row still
     *  names the clip (that is how it says there *was* audio), and the
     *  server reports whether the file is still there as `clip_kept`. A
     *  record pushed over the websocket has no `clip_kept` -- it was
     *  written seconds ago, so it is kept. */
    _transcript(record) {
      const status = record.transcript_status;
      if (!status) return null;

      const wrap = el("div", "hr-transcript");
      const removed = Boolean(record.clip) && record.clip_kept === false;
      if (record.clip && !removed) {
      ...
      if (removed) {
        wrap.appendChild(el("small", "hr-noclip", "audio clip removed due to retention settings"));
      }
      return wrap;
    }
