    ...
    return web.json_response(
        {
            ...
            "keep_clips_bounds": [0, transcribe.MAX_KEEP_CLIPS],
            "keep_clips_mb_bounds": [0, transcribe.MAX_KEEP_CLIPS_MB],
            "keep_clips_days_bounds": [0, transcribe.MAX_KEEP_CLIPS_DAYS],
        }
    )
...
async def get_history(request: web.Request) -> web.Response:
    ...
    records = history.records(
        limit=max(1, min(limit, MAX_HISTORY_PAGE)), offset=max(0, offset), **filters
    )
    # Whether the audio a row names is still there. A row keeps its clip
    # name after the ClipStore has pruned the file, so without this the play
    # button is on every row that ever had audio and fails on the old ones
    # -- which reads as "the recordings are broken" rather than "the
    # retention setting is 3000 calls". One stat per row on the page.
    transcriber = getattr(request.app[MANAGER_KEY], "transcriber", None)
    for record in records:
        if record.get("clip"):
            record["clip_kept"] = (
                transcriber.clips.kept(record["clip"]) if transcriber is not None else False
            )
