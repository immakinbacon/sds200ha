...
class TestReceiveHistory(unittest.TestCase):
    ...
    def test_an_offset_turns_the_page(self):
        for i in range(5):
            self.store.status_callback("home", self._status(TGID={"Name": f"tg{i}", "TGID": str(i)}))
            self.clock.tick()
        pages = [
            [r["tgid"] for r in self.store.records(limit=2, offset=offset)]
            for offset in (0, 2, 4, 6)
        ]
        self.assertEqual(pages, [["4", "3"], ["2", "1"], ["0"], []])
        # Filters and the offset compose: page two *of the matches*.
        self.assertEqual(
            [r["tgid"] for r in self.store.records(limit=1, offset=1, query="tg")],
            ["3"],
        )

    def test_clips_die_with_their_rows(self):
        reaped = []
        self.store.clip_reaper = reaped.append
        for i in range(8):
            self.store.status_callback("home", self._status(TGID={"Name": str(i), "TGID": str(i)}))
            self.clock.tick()
        for record in self.store.records():
            self.store.set_transcript(record["id"], "", "no-speech", clip=f"{record['id']}.wav")
        oldest = [r["id"] for r in self.store.records()][-3:]
        # max_records is 5, so the cap drops the three oldest -- and hands
        # their clips over in the same pass.
        self.store.prune(force=True)
        self.assertEqual(len(self.store.records()), 5)
        self.assertEqual(sorted(reaped[0]), sorted(f"{i}.wav" for i in oldest))

    def test_old_clips_are_reaped_while_their_rows_stay(self):
        reaped = []
        self.store.clip_reaper = lambda names: reaped.extend(names) or len(names)
        self._record_one_call()
        first = self.store.records()[0]["id"]
        self.store.set_transcript(first, "engine twelve", "ok", clip=f"{first}.wav")
        self.clock.tick(3 * history.DAY_S)
        self._record_one_call()
        second = self.store.records()[0]["id"]
        self.store.set_transcript(second, "medic four", "ok", clip=f"{second}.wav")

        self.store.configure(enabled=True, max_records=5, retention_days=90, clip_days=2)
        self.assertEqual(reaped, [f"{first}.wav"])
        # The row is still there, transcript and clip name intact: the page
        # says the audio was removed, not that there never was any.
        row = self.store.record(first)
        self.assertEqual(row["transcript"], "engine twelve")
        self.assertEqual(row["clip"], f"{first}.wav")
        # And it is not handed over again on the next pass.
        self.store.prune(force=True)
        self.assertEqual(reaped, [f"{first}.wav"])
        # Until the second call ages past the cutoff too.
        self.clock.tick(3 * history.DAY_S)
        self.store.prune(force=True)
        self.assertEqual(reaped, [f"{first}.wav", f"{second}.wav"])
