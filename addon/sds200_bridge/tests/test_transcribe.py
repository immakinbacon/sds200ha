...
class TestClipRetention(unittest.TestCase):
    def setUp(self):
        ...
        self.addCleanup(self._tmp.cleanup)
        self.clips = ClipStore(os.path.join(self._tmp.name, "clips"), keep=5)
        for call_id in range(1, 7):
            self.clips.save(call_id, b"\xff" * 160)

    def test_the_oldest_go_first_and_the_row_can_tell(self):
        self.assertEqual(sorted(self.clips.names()), ["2.wav", "3.wav", "4.wav", "5.wav", "6.wav"])
        self.assertFalse(self.clips.kept("1.wav"))
        self.assertTrue(self.clips.kept("2.wav"))
        self.assertFalse(self.clips.kept(None))

    def test_lowering_the_setting_prunes_at_once(self):
        # On save, not on the next call: the disk is freed when the person
        # asked for it to be.
        self.assertEqual(self.clips.set_limits(keep=2, max_mb=0), 3)
        self.assertEqual(sorted(self.clips.names()), ["5.wav", "6.wav"])
        self.assertEqual(self.clips.set_limits(keep=50, max_mb=0), 0)

    def test_zero_keeps_them_all(self):
        self.clips.set_limits(keep=0, max_mb=0)
        for call_id in range(7, 20):
            self.clips.save(call_id, b"\xff" * 160)
        self.assertEqual(len(self.clips.names()), 18)

    def test_the_disk_budget_drops_the_oldest(self):
        # Each clip is 160 samples of 16-bit PCM plus a 44-byte header. The
        # budget is in whole megabytes, so make the clips big enough that
        # one megabyte holds a known number of them.
        clips = ClipStore(os.path.join(self._tmp.name, "big"), keep=0, max_bytes=0)
        for call_id in range(1, 5):
            clips.save(call_id, b"\xff" * (300 * 1024))  # ~600kB of PCM each
        self.assertEqual(len(clips.names()), 4)
        self.assertEqual(clips.size(), sum(
            os.path.getsize(clips.path(n)) for n in clips.names()
        ))
        # 1MB holds one ~600kB clip, not two.
        self.assertEqual(clips.set_limits(keep=0, max_mb=1), 3)
        self.assertEqual(clips.names(), ["4.wav"])
        # And the running total tracked every unlink, so the next decision
        # is made on the true size without another walk of the directory.
        self.assertEqual(clips.size(), os.path.getsize(clips.path("4.wav")))
        clips.save(5, b"\xff" * (300 * 1024))
        self.assertEqual(clips.names(), ["5.wav"])
